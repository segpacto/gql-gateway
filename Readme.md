...
### Publish available Rest Endpoints as GraphQl
#### Simpliest scenario :
```js
...
At the same time graphql queries and mutations are automatically generated, allowing to retrieve from the REST APIs declared above using graphql ways.

#### Advanced sceario (Adding aggregations):
```js
const gateway = require('gql-gateway')
...
Explanation from Top to bottom :
    On `localSchema` we declare the aggregations that we would like to do by extending the original schemas(To get the original schemas, queries and mutations is recommended to publish the service and then take a look at them before start adding aggregations). 
...
| `apolloServerConfig` | `[]`                        | Apollo Server configuration ((https://www.apollographql.com/docs/apollo-server/api/apollo-server/#apolloserver))|
...
| `logger`             | `console`                   | If true, all logs are suppressed |
...
Below, we describe how to interact between services using agreggations(relations). In this case we use the `User` and `Product` services.
